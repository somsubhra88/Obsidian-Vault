...
The input data is a list of JSON and each JSON contains four primary keys - ASIN, ProductTitle, ReleaseDate, and ProductGroup. ASIN is a mandatory field and the rest of the fields are strings. We're following the schema to validate the incoming JSON. We don't require any transformation on this data, we just convert the data into a pandas data frame and from there we store it as a comma-separated file.
```JSON
schema = {  
    'ASIN': {  
        'type': 'string',  
